...
Once a database has been set for [I18n](https://github.com/qor/i18n), all **untranslated** translations inside `I18n.T()` will be loaded into `translations` table in the database when compiling the application. For example, we have an untranslated `I18n.T("en-US", "demo.greeting")` in the example, so [I18n](https://github.com/qor/i18n) will generate this record in the `translations` table after compiling.
...
[I18n](https://github.com/qor/i18n) has a built-in web interface for translations which is integrated with [QOR Admin](../chapter2/setup.md).
...
The easy way to use [I18n](https://github.com/qor/i18n) in a template is to define a `t` function and register it as `FuncMap`:
...
[I18n](https://github.com/qor/i18n) has a `Fallbacks` function to register fallbacks. For example, registering `en-GB` as a fallback to `zh-CN`:
...
[I18n](https://github.com/qor/i18n) utilizes a Golang template to parse translations with an interpolation variable.
